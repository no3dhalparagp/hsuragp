...
export default function AwardOfContractClient({
  ...
  const notDelivered = useMemo(() => {
    return workOrders.filter(
      (order) =>
        !order.awardofcontractdetails?.isdelivery &&
        matchesSearch(order)
    );
  }, [workOrders, search]);
  ...
  function matchesSearch(order: any) {
    ...
    const agency =
      order.Bidagency?.agencydetails?.name?.toLowerCase() || "";
    ...
    <TableRow key={order.id} className="hover:bg-blue-50">
      ...
      <TableCell className="text-blue-700 font-medium">
        <ShowNitDetails
          ...
          memoDate={
            order.Bidagency?.WorksDetail?.nitDetails?.memoDate
          }
  ...
  return (
    <div className="min-h-screen bg-blue-50">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        {errorMessage && (
          <StatusAlert type="error" message={errorMessage} />
        )}
