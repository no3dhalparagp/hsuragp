...
const Page = async ({ params }: { params: { workid: string[] } }) => {
  ...
  const qualifiedCount = technical.filter(item => item.technicalEvelution?.qualify).length;
  const allEvaluationsSubmitted = technical.every(item => item.technicalEvelutiondocumentId !== null);
